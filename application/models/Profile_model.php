...
class Profile_model extends CI_Model
{
     ...
     public function GetAllfeeds($userId)
    {
     ...
     public function GetAllfeeds_limit($userId,$fi,$la)
    {
        $this->db->select('DISTINCT(us.user_id),fi.image_name,us.gender,up.profile_pic,uf.created_at,up.nick_name,us.full_name,uf.feeds,uf.no_likes,uf.id,up.country_id,cf.country');
        $this->db->from('users us');
        $this->db->join('friends um', 'um.friend_id=us.user_id', 'LEFT');
    ...
    public function GetAllfeedss($userId)
    {
          $this->db->select('DISTINCT(us.user_id),fi.image_name,us.gender,up.profile_pic,uf.created_at,up.nick_name,us.full_name,uf.feeds,uf.no_likes,uf.id,up.country_id,cf.country');
        $this->db->from('users us');
 ...
 $query = $this->db->query("INSERT INTO user_feed_image (image_name ,feed_id)
	...
	public function update_friendlist_privacy($uid,$visibility)
    {   
        $query = $this->db->query("UPDATE user_profile SET friendlist_hide='$visibility' WHERE user_id =$uid");
        return true;  
    }
